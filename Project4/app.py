...
def train_model_with_ui(model, X_train, y_train, X_val, y_val, epochs, learning_rate, batch_size, device):
    ...
    for epoch in range(epochs):
        ...
        if epoch > 0:  # Only show after first epoch
            time_text.info(f"⏱️ Epoch time: {epoch_time:.1f}s | Estimated remaining: {estimated_remaining:.1f}s ({estimated_remaining/60:.1f} min)")
...
def main():
    ...
    **🚀 Auto-Training Mode:** This app automatically trains the model when loaded and displays the results. 
    ...
    with tab5:
        ...
        **Challenges:**
        - ⚠️ Sudden weather changes difficult to predict
        - ⚠️ Long-term forecasts (>1 day) less accurate
        - ⚠️ Extreme temperatures underrepresented in training
        - ⚠️ Model learns general trends but misses rapid fluctuations
